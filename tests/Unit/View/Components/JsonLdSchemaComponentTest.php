...
describe('JsonLdSchema', function () {
    it('renders JSON-LD schema with default values', function () {
        $html = view('components.layouts.common.json-ld-schema')->render();
    ...
    it('renders with custom type', function () {
        $html = view('components.layouts.common.json-ld-schema', ['type' => 'Article'])->render();
        expect($html)->toContain('"Article"');
    });

    it('renders with custom author array', function () {
        $author = ['@type' => 'Organization', 'name' => 'Test Org'];
        $html = view('components.layouts.common.json-ld-schema', ['type' => 'WebSite', 'author' => $author])->render();
